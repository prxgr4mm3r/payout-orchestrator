...
func (w *Worker) Run(ctx context.Context) error {
	...
	if w.queueName == "" {
		return errors.New("webhook worker queue name is required")
	}
...
func (w *Worker) handleDelivery(ctx context.Context, delivery platformrabbitmq.Delivery) error {
	event, err := rabbitmqbroker.DecodeWebhookDeliveryJob(delivery.Body())
	if err != nil {
		if nackErr := delivery.Nack(false); nackErr != nil {
			return nackErr
		}

		w.logger.Printf("dropped malformed webhook message: %v", err)
		return nil
	}
	...
	if err := w.handler.HandleEvent(ctx, event); err != nil {
		...
		w.logger.Printf("webhook message handling failed event_id=%s type=%s err=%v", event.ID, event.EventType, err)
		return nil
	}
	...
	w.logger.Printf("processed webhook message event_id=%s type=%s", event.ID, event.EventType)
	return nil
}
